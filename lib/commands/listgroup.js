...
module.exports = {
  ...
  async* run(session, cmd) {
    ...
    yield `${status._211_GRP_SELECTED} ${g.total} ${g.min_index} ${g.max_index} ${name} list follows`;

    for await (let article of session.server._getRange(session, first, last)) {
      yield article.index.toString();
    }
